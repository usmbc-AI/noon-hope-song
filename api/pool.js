...
module.exports = async (req, res) => {
  ...
  const krArtists = [...new Set([...shuffle(krC.artists).slice(0,11), ...shuffle(MAJOR_EVERGREEN).slice(0,3), ...shuffle(INDIE_POOL).slice(0,6)])].slice(0,20);
  const popArtists = [...new Set([...shuffle(usC.artists).slice(0,4), ...shuffle(POP_ARTISTS).slice(0,5)])].slice(0,8);
