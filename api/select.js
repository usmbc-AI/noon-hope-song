...
${hasUserMood && userMoods.length >= 2 ? `[첫 곡 특별 미션 — 재치있게!]
- songs의 **첫 번째 곡**은 무드 키워드 ${userMoods.map(m => `"${m}"`).join(", ")} 를 **하나도 빠짐없이 모두** 엮어낼 수 있는 곡으로 고르세요.
- 다소 억지스러워도 좋습니다. 언어유희·유머·비유·과장을 총동원해 재치있게 연결하세요.
- 그 첫 곡의 reason에는 세 키워드를 전부 녹인 **그럴듯하고 위트있는 1~2문장**을 써주세요. (일반 곡 이유 형식보다 이 재치를 우선합니다.)
  (아래는 '톤' 참고용 예시일 뿐입니다. **문장을 그대로 베끼지 말고, 실제 키워드에 맞춰 완전히 새로 창작하세요.**)
  (예시 톤 — 키워드가 '무더위, 트럼프, 냉면'이라면: "숨 막히는 무더위 속 트럼프가 들이킨 살얼음 낀 냉면 육수처럼, 복잡한 국제 정세의 이면을 꿰뚫는 이 곡의 차가운 한 방이 식도를 파고듭니다.")
