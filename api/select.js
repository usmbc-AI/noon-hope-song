...
${hasUserMood && userMoods.length >= 2 ? `[첫 곡·둘째 곡 특별 미션 — 키워드 종합 (진지 + 라이트)]
무드 키워드 ${userMoods.map(m => `"${m}"`).join(", ")} 를 **마인드맵처럼 확장**해, 첫 곡과 둘째 곡 둘 다 키워드 전체를 엮되 **톤을 다르게** 하세요. (두 곡은 서로 다른 곡·다른 아티스트)

▶ songs[0] = **진지 버전** (bridge: true)
- 키워드들을 관통하는 하나의 공통 개념/정서를 진지하게 도출.
- reason은 **확장 과정을 자세히** 4~6문장으로: (1) 각 키워드가 어떤 이미지·정서로 뻗는지 하나씩 펼치고, (2) 어디서 겹쳐 '○○'로 수렴하는지 연결 과정을 설명한 뒤, (3) "그래서" 이 곡이 왜 맞는지 마무리.
  (예시 톤 — '무더위,트럼프,냉면': "'무더위'는 끈적한 열기와 짜증으로, '트럼프'는 예측불가한 도발과 긴장으로 뻗고, '냉면'은 그 뜨거움을 살얼음으로 눌러 식히는 쾌감으로 이어집니다. 세 갈래는 결국 '달아오른 것을 시원하게 터뜨리는 카타르시스'에서 만나죠. 그래서 속을 뻥 뚫는 이 곡이…")

▶ songs[1] = **라이트 버전** (bridgeLight: true)
- 같은 키워드를, 진지 대신 **가볍고 유머러스하게**! 억지스러운 비약·드립·말장난·과장 대환영 (읽으면 피식 웃어넘길 수준으로 우겨넣기 OK).
- reason은 진지 버전처럼 **4~6문장으로 길고 자세하게** 확장 과정을 보여주세요(짧게 쓰지 말 것, 길이는 진지 버전과 비슷하게). 톤만 유쾌하게: (1) 각 키워드를 능청스럽고 엉뚱하게 펼치고, (2) 다소 억지로라도 우겨서 하나의 '○○'로 엮은 뒤, (3) "그래서 이 곡!" 하고 뻔뻔하게 마무리.
  (예시 톤 — '무더위,트럼프,냉면': "자, 냉정하게 봅시다. 무더위는 우릴 사정없이 녹이고, 트럼프는 뉴스 볼 때마다 혈압을 녹이는데, 냉면은... 어? 얘만 우릴 살려주네요? 여기서 대반전. 셋의 공통점은 '녹는다'가 아니라 '이 여름 시원한 육수 한 사발이면 세계 평화도 지킨다'는 원대한 진리였던 겁니다. 트럼프도 냉면 앞에선 트윗을 잠시 멈출 테니까요. 그래서 오늘은 복잡한 생각 다 말아먹고, 젓가락 대신 리듬을 집어 이 곡에 몸을 맡기는 겁니다!")

- 나머지(3번곡~)는 평소대로 무드에 맞게.

※ 두 버전 모두 지킬 것: 키워드를 잇는 '해석·연결'은 억지·유머 대환영이지만, **곡 자체의 장르·분위기 묘사는 후보 목록에 주어진 실제 장르·태그에 맞게 정확히** 쓰세요. 없는 장르·특성을 지어내지 마세요. (예: 발라드 곡인데 '힙합 기반'이라고 하면 안 됨. 모르면 장르를 단정하지 말고 곡의 무드만 언급.)
...
- 장르·특성은 **주어진 장르·태그와 어긋나게 단정하지 마세요**(확실치 않으면 곡의 무드만 언급). 실제와 다른 장르를 지어내면 안 됩니다.
- 이 reason은 **화면 표시 + AI DJ가 음성으로 읽는 소개 글**로 함께 쓰입니다. 이모지·해시태그·특수기호는 넣지 마세요(소리내어 읽힘). 곡을 앞으로 소개하는 현재형/청유형으로 쓰고, '들려드렸습니다' 같은 과거형은 쓰지 마세요.
